...
namespace path_finder
{
	...
	GraphObserver& GraphObserver::Instance(void){
	...
	GraphObserver::GraphObserver(void) {}

	GraphObserver::GraphObserver(const GraphObserver& that){
		// since it is static class this body is empty/not needed*/
	}

	GraphObserver& GraphObserver::operator= (const GraphObserver&){
		// since it is static class this body is empty/not needed*/
		return GraphObserver::_instance;
	}
	...
	GraphObserver::~GraphObserver(void) noexcept{
		...
		// TO DO ---> UNCOMMENT THE JOIN*INSTRUCTION"
